...
//===----------------------------------------------------------------------===//
  ...
  #include "llvm/ADT/Extras.h"
  #include "llvm/IR/Extras.h"
#endif
...
bool DependenceInfo::propagate(const SCEV *&Src, const SCEV *&Dst,
  ...
  bool Result = false;
#if (LLVM_VERSION_MAJOR < 9)
  for (unsigned LI : sbv_set_bits(&Loops)) {
#else
...
static void dumpSmallBitVector(SmallBitVector &BV) {
  dbgs() << "{";
#if (LLVM_VERSION_MAJOR < 9)
  for (unsigned VI : sbv_set_bits(&BV)) {
#else
...
#if (LLVM_VERSION_MAJOR < 9)
  for (unsigned SI : sbv_set_bits(&Separable)) {
#else
  ...
  if (Coupled.count()) {
...
#if (LLVM_VERSION_MAJOR < 9)
    for (unsigned SI : sbv_set_bits(&Coupled)) {
#else
    for (unsigned SI : Coupled.set_bits()) {
...
#if (LLVM_VERSION_MAJOR < 9)
      for (unsigned SJ : sbv_set_bits(&Group)) {
#else
      ...
      while (Sivs.any()) {
        bool Changed = false;
#if (LLVM_VERSION_MAJOR < 9)
        for (unsigned SJ : sbv_set_bits(&Sivs)) {
#else
...
#if (LLVM_VERSION_MAJOR < 9)
          for (unsigned SJ : sbv_set_bits(&Mivs)) {
#else
      ...
      DEBUG(dbgs() << "    updating\n");
#if (LLVM_VERSION_MAJOR < 9)
      for (unsigned SJ : sbv_set_bits(&ConstrainedLevels)) {
#else
