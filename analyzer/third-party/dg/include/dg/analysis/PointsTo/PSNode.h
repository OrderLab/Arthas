...
class PSNode : public SubgraphNode<PSNode> {
 ...
 protected:
  ...
  PSNode(unsigned id, PSNodeType t) : SubgraphNode<PSNode>(id), type(t) {
    switch (type) {
      ...
      case PSNodeType::FUNCTION:
        ...
        // std::cout << "psnode func add pt \n";
        // std::cout << "Func is " << id << "\n";
        addPointsTo(this, 0);
        break;
      case PSNodeType::CALL:
        // std::cout << "This is just a regular call node " << id << "\n";
        // addPointsTo(this,0);
        // the call without arguments
        break;
      default:
        // std::cout << "This is a default" << id << "\n";
        break;
    }
  }

  // ctor for memcpy
  PSNode(unsigned id, PSNodeType t, PSNode *op1, PSNode *op2) : PSNode(id, t) {
  ...
  // ctor for gep
  PSNode(unsigned id, PSNodeType t, PSNode *op1) : PSNode(id, t) {
    assert(t == PSNodeType::GEP);
    addOperand(op1);
  }

  // ctor of constant
  PSNode(unsigned id, PSNodeType t, PSNode *op, Offset offset) : PSNode(id, t) {
  ...
  PSNode(unsigned id, PSNodeType t, va_list args) : PSNode(id, t) {
    PSNode *op;
 ...
 public:
  PSNode(PSNodeType t) : PSNode(0, t) {
    switch (t) {
  ...
  // convenient helper
  bool addPointsTo(PSNode *n, Offset o) { /*std::cout << "p1\n";*/
    return pointsTo.add(Pointer(n, o));
  }
  bool addPointsTo(const Pointer &ptr) { /*std::cout << "p2\n";
                                            if(ptr.isUnknown()) {std::cout <<
                                            "unknown p2\n";}*/
    return pointsTo.add(ptr);
  }
  bool addPointsTo(const PointsToSetT &ptrs) { /*std::cout << "p3\n";*/
    return pointsTo.add(ptrs);
  }
  bool addPointsTo(
      std::initializer_list<Pointer> ptrs) { /*std::cout << "p4\n";*/
    return pointsTo.add(ptrs);
  }
  ...
  PSNode *stripCasts() {
    PSNode *node = this;
    while (node->getType() == PSNodeType::CAST) node = node->getOperand(0);
...
class PSNodeRet : public PSNode {
 ...
 public:
  PSNodeRet(unsigned id, va_list args) : PSNode(id, PSNodeType::RETURN, args) {}
...
class PSNodeFork : public PSNode {
 ...
 public:
  PSNodeFork(unsigned id) : PSNode(id, PSNodeType::FORK) {}
...
class PSNodeJoin : public PSNode {
 ...
 public:
  PSNodeJoin(unsigned id) : PSNode(id, PSNodeType::JOIN) {}
