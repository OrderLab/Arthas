...
class LLVMPointerGraphBuilder {
  ...
  class PSNodesSeq {
   ...
   public:
    ...
    PSNodesSeq(const std::initializer_list<PSNode *> &l) {
      for (auto n : l) _nodes.push_back(n);
    }
  ...
  void PSNodesBlockAddSuccessors(PSNodesBlock &blk, bool withSeqEdges = false) {
    ...
    for (auto seq : blk) {
      if (withSeqEdges) PSNodesSequenceAddSuccessors(*seq);

      if (last) last->getLast()->addSuccessor(seq->getFirst());
  ...
  void addCFGEdges(const llvm::Function *F,
                   LLVMPointerGraphBuilder::FuncGraph &finfo, PSNode *lastNode);
 ...
 public:
  ...
  const std::unordered_map<const llvm::Value *, PSNodesSeq> &getNodesMap()
      const {
    return nodes_map;
  }
 ...
 private:
  ...
  PSNodesSeq *getNodes(const llvm::Value *val) {
    ...
    if (it == nodes_map.end()) return nullptr;
  ...
  void addArgumentOperands(const llvm::InvokeInst *CI, PSNode *arg, int idx);
  void addArgumentOperands(const llvm::CallInst &CI, PSNode &node);
  void addArgumentOperands(const llvm::InvokeInst &CI, PSNode &node);
  void addArgumentsOperands(const llvm::Function *F,
                            const llvm::CallInst *CI = nullptr, int index = 0);
  void addArgumentsOperands(const llvm::Function *F,
                            const llvm::InvokeInst *CI = nullptr,
                            int index = 0);
  ...
  void addVariadicArgumentOperands(const llvm::Function *F,
                                   const llvm::InvokeInst *CI, PSNode *arg);
  ...
  void addInterproceduralOperands(const llvm::Function *F,
                                  ...
                                  PSNode *callNode = nullptr);
  void addInterproceduralOperandsInvoke(const llvm::Function *F,
                                        PointerSubgraph &subg,
                                        const llvm::InvokeInst *CI = nullptr,
                                        PSNode *callNode = nullptr);
  ...
  PSNodesSeq &createFunctionInvoke(const llvm::InvokeInst *,
                                   const llvm::Function *);
  PSNodesSeq &createFuncptrInvoke(const llvm::InvokeInst *,
                                  const llvm::Value *);
  PSNodesSeq &createInvokeToFunction(const llvm::InvokeInst *,
                                     const llvm::Function *);
  ...
  PointerSubgraph &getAndConnectSubgraph(const llvm::Function *F,
                                         ...
                                         PSNode *callNode);
  PointerSubgraph &getAndConnectSubgraphInvoke(const llvm::Function *F,
                                               const llvm::InvokeInst *CInst,
                                               PSNode *callNode);

  void handleGlobalVariableInitializer(const llvm::Constant *C,
                                       PSNodeAlloc *node, uint64_t offset = 0);
  ...
  PSNodesSeq &createDynamicMemAlloc(const llvm::CallInst *CInst,
                                    AllocationFunction type);
  PSNodesSeq &createDynamicMemAllocInvoke(const llvm::InvokeInst *CInst,
                                          AllocationFunction type);
  PSNodesSeq &createRealloc(const llvm::CallInst *CInst);
  PSNodesSeq &createUnknownCall(const llvm::CallInst *CInst);
  PSNodesSeq &createUnknownCall(const llvm::InvokeInst *CInst);
...
inline bool isInvalid(const llvm::Value *val, bool invalidate_nodes) {
  ...
  if (!isa<Instruction>(val)) {
    if (!isa<Argument>(val) && !isa<GlobalValue>(val)) return true;
  } else {
    if (isa<ICmpInst>(val) || isa<FCmpInst>(val) || isa<DbgValueInst>(val) ||
        isa<BranchInst>(val) || isa<SwitchInst>(val))
      return true;
