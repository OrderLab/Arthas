...
// in the map
bool BasicRDMap::merge(const BasicRDMap* oth, DefSiteSetT* no_update,
                       bool strong_update_unknown, Offset::type max_set_size,
                       bool merge_unknown) {
  ...
  for (okit = oth->key_begin(), okend = oth->key_end(); okit != okend; ++okit) {
    ...
    if (no_update) {  // do we have anything for strong update at all?
      ...
      if (strong_update_unknown && is_unknown && ds.target->getSize() > 0) {
        ...
        auto range =
            std::equal_range(no_update->begin(), no_update->end(), ds, comp_ds);
        ...
        if (overwrites_whole_memory) continue;
    ...
    if (merge_unknown && is_unknown) {
      ...
      // after the loop exit
      our_vals = &(*this)[DefSite(ds.target, Offset::UNKNOWN, Offset::UNKNOWN)];

      auto range = getObjectRange(ds);

      std::set<DefSite> erased;
      for (auto I = range.first; I != range.second;) {
        ...
        if (&cur->second == our_vals) continue;
        ...
        erased.insert(cur->first);
        _defs.erase(cur);
      }
      if (!erased.empty()) {
        key_iterator kit = _keys.begin();
        while (kit != _keys.end()) {
          if (erased.find(*kit) != erased.end()) {
            kit = _keys.erase(kit);
          } else {
            ++kit;
          }
