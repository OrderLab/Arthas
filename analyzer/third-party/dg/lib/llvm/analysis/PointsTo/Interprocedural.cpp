...
void LLVMPointerGraphBuilder::addArgumentOperands(const llvm::InvokeInst *CI,
                                                  PSNode *arg, int idx) {
  assert(idx < static_cast<int>(CI->getNumArgOperands()));
  PSNode *op = tryGetOperand(CI->getArgOperand(idx));
  if (op && !arg->hasOperand(op)) {
...
void LLVMPointerGraphBuilder::addArgumentOperands(const llvm::Function *F,
  ...
  for (auto I = F->use_begin(), E = F->use_end(); I != E; ++I) {
    ...
    if (CI && CI->getCalledFunction() == F) addArgumentOperands(CI, arg, idx);
  }
}
...
void LLVMPointerGraphBuilder::addVariadicArgumentOperands(
    const llvm::Function *F, const llvm::InvokeInst *CI, PSNode *arg) {
  for (unsigned idx = F->arg_size() - 1; idx < CI->getNumArgOperands(); ++idx)
    addArgumentOperands(CI, arg, idx);
}
...
void LLVMPointerGraphBuilder::addReturnNodeOperand(PSNode *callNode,
  ...
  if (!callReturn->hasOperand(ret)) callReturn->addOperand(ret);
...
void LLVMPointerGraphBuilder::addInterproceduralOperands(
    const llvm::Function *F, PointerSubgraph &subg, const llvm::CallInst *CI,
    PSNode *callNode) {
  ...
  if (!subg.returnNodes.empty()) {
    ...
    if (callNode->successorsNum() == 1 &&
        callNode->getSingleSuccessor() == callReturnNode) {
      callNode->removeSingleSuccessor();
    }
  }
}
void LLVMPointerGraphBuilder::addInterproceduralOperandsInvoke(
    const llvm::Function *F, PointerSubgraph &subg, const llvm::InvokeInst *CI,
    PSNode *callNode) {
  assert((!CI || callNode) && (!callNode || CI));
