...
RDNode *LLVMRDBuilder::createAlloc(const llvm::Instruction *Inst) {
  ...
  if (iterator == nodes_map.end()) {
    ...
    assert(iterator->second->getType() == RDNodeType::CALL &&
           "Adding node we already have");
    addArtificialNode(Inst, node);
    makeEdge(iterator->second, node);
  }

  if (const llvm::AllocaInst *AI = llvm::dyn_cast<llvm::AllocaInst>(Inst))
    node->setSize(getAllocatedSize(AI, DL));

  return node;
}

RDNode *LLVMRDBuilder::createDynAlloc(const llvm::Instruction *Inst,
  ...
  for (const BasicBlock &block : *F) {
    for (const Instruction &Inst : block) {
      if (isa<AllocaInst>(&Inst)) {
        ...
        if (!is_address_taken) ret.insert(&Inst);
      }
    }
  }
}
...
static bool isRelevantCall(const llvm::Instruction *Inst, OptsT &opts) {
  ...
  if (func->size() == 0) {
    ...
    if (opts.getFunctionModel(func->getName())) return true;
    // memory allocation
    if (opts.isAllocationFunction(func->getName())) return true;
...
LLVMRDBuilder::Block &LLVMRDBuilder::buildBlockNodes(
    Subgraph &subg, const llvm::BasicBlock &llvmBlock) {
  using namespace llvm;
...
LLVMRDBuilder::Block &LLVMRDBuilder::buildBlock(
    Subgraph &subg, const llvm::BasicBlock &llvmBlock) {
...
void LLVMRDBuilder::blockAddSuccessors(
    LLVMRDBuilder::Subgraph &subg, LLVMRDBuilder::Block &block,
    const llvm::BasicBlock *llvmBlock,
    std::set<const llvm::BasicBlock *> &visited) {
  assert(!block.nodes.empty() && "Block is empty");

  for (auto S = llvm::succ_begin(llvmBlock), SE = llvm::succ_end(llvmBlock);
       S != SE; ++S) {
    ...
    if ((succIt == subg.blocks.end() || succIt->second.nodes.empty())) {
...
LLVMRDBuilder::Subgraph *LLVMRDBuilder::getOrCreateSubgraph(
...
std::pair<RDNode *, RDNode *> LLVMRDBuilder::createCallToFunction(
    const llvm::Function *F, const llvm::CallInst *CInst) {
  ...
  if (auto model = _options.getFunctionModel(F->getName())) {
    ...
    addNode(CInst, node);
    return {node, node};
  } else if (F->size() == 0 || _options.entryOnly) {
    ...
    llvm::errs() << "[RD] error: call of incompatible function: "
                 << ValInfo(CInst) << "\n";
    llvm::errs() << "            Calling : " << F->getName() << " of type "
                 << *F->getType() << "\n";
...
std::pair<RDNode *, RDNode *> LLVMRDBuilder::createCallToFunctions(
  ...
  if (!incompatibleCalls.empty()) {
    ...
    if (incompatibleCalls.size() == functions.size()) {
      llvm::errs()
          << "[RD] error: did not find any compatible pointer for this call.\n";
    }
#else
    if (incompatibleCalls.size() == functions.size()) {
      llvm::errs()
          << "[RD] error: did not find any compatible function pointer for "
          << ValInfo(CInst) << "\n";
      for (auto F : incompatibleCalls) {
        llvm::errs() << "   Tried call: " << F->getName() << " of type "
                     << *F->getType() << "\n";
...
static std::vector<const llvm::BasicBlock *> getBasicBlocksInDominatorOrder(
    llvm::Function &F) {
...
LLVMRDBuilder::Subgraph &LLVMRDBuilder::buildFunction(const llvm::Function &F) {
...
RDNode *LLVMRDBuilder::createUndefinedCall(const llvm::CallInst *CInst) {
  ...
  if (_options.undefinedArePure) return node;
...
RDNode *LLVMRDBuilder::createIntrinsicCall(const llvm::CallInst *CInst) {
  ...
  for (const auto &ptr : pts.second) {
    if (llvm::isa<llvm::Function>(ptr.value)) continue;
...
RDNode *LLVMRDBuilder::funcFromModel(const FunctionModel *model,
                                     const llvm::CallInst *CInst) {
  RDNode *node = create(RDNodeType::CALL);

  for (unsigned int i = 0; i < CInst->getNumArgOperands(); ++i) {
    if (!model->handles(i)) continue;
...
std::pair<RDNode *, RDNode *> LLVMRDBuilder::createCall(
...
RDNode *LLVMRDBuilder::createCallToZeroSizeFunction(
...
ReachingDefinitionsGraph &&LLVMRDBuilder::build() {
  ...
  if (!F) {
    llvm::errs() << "The entry function was not found in the module\n";
    abort();
  }
  ...
  for (auto &it : calls) {
    ...
    assert(returnNode->getType() == RDNodeType::CALL_RETURN &&
           "Do not have return node for a call");
