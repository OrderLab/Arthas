...
bool isConstantValue(llvm::Value const *const V) {
  return isa<ConstantPointerNull>(V) ||
         (isa<Constant const>(V) && !isa<GlobalValue>(V) &&
          !isa<UndefValue>(V));
}
...
bool isPointerManipulation(llvm::Instruction const *const I) {
  if (isa<AllocaInst>(I)) {
    ...
    if (llvm::dyn_cast<llvm::PointerType const>(I->getOperand(0)->getType())
            ->getElementType()
            ->isPointerTy())
      return true;
  ...
  assert(!isPointerValue(I) && "Instruction cannot be a of pointer type here!");
...
bool memoryManStuff(llvm::Value const *const V) {
  return isMemoryAllocation(V) || isMemoryDeallocation(V) || isMemoryCopy(V) ||
         isMemoryMove(V) || isMemorySet(V);
}
...
const Value *elimConstExpr(const Value *V) {
  if (const ConstantExpr *CE = dyn_cast<ConstantExpr>(V)) {
    if (Instruction::isBinaryOp(CE->getOpcode()))
      return V;
    assert(
        (CE->getOpcode() == llvm::Instruction::GetElementPtr || CE->isCast()) &&
