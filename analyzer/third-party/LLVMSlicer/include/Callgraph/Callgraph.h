...
struct Callgraph {
  ...
  Callgraph(const Module &M, const llvm::ptr::PointsToSets &PS);
...
void computeTransitiveClosure(Relation const &R, Relation &TCR) {
  ...
  typedef std::multimap<typename Relation::value_type::first_type,
                        typename Relation::value_type::second_type>
      Dict;
...
static inline Callgraph::range_iterator getCalls(Callgraph::key_type const &key,
                                                 Callgraph const &CG) {
  return CG.calls(key);
}
